...
def create_prereq_material(material_name, material):
    ...
    elif material_name == PUMPKIN_SKIN_MAT_NAME:
...
def create_prereq_node_group(node_group_name, node_tree_type):
    if node_tree_type == 'GeometryNodeTree':
        ...
        elif node_group_name == PUMPKIN_DEFORM_GEO_NG_NAME:
...
def create_mat_ghost(material):
    ...
    node.inputs[1].default_value = (0.42, -0.8, 1.33)
    ...
    node.operation = "SUBTRACT"
    node.inputs[1].default_value = (0.0, -1.0, 0.8)
    node.inputs[2].default_value = (0.0, 0.0, 0.0)
    node.inputs[3].default_value = 1.000000
    ...
    tree_links.new(new_nodes["Translucent BSDF"].outputs[0], new_nodes["Mix Shader"].inputs[1])
    ...
    tree_links.new(new_nodes["Texture Coordinate.001"].outputs[3], new_nodes["Vector Math.002"].inputs[0])
    tree_links.new(new_nodes["Vector Math.002"].outputs[0], new_nodes["Vector Math.003"].inputs[0])
    tree_links.new(new_nodes["Vector Math.003"].outputs[1], new_nodes["Math.002"].inputs[0])
    tree_links.new(new_nodes["Math.002"].outputs[0], new_nodes["Math.001"].inputs[1])
    tree_links.new(new_nodes["Mix Shader.002"].outputs[0], new_nodes["Add Shader"].inputs[1])
    tree_links.new(new_nodes["Transparent BSDF"].outputs[0], new_nodes["Mix Shader.002"].inputs[1])
    tree_links.new(new_nodes["Mix Shader"].outputs[0], new_nodes["Add Shader"].inputs[0])
    tree_links.new(new_nodes["Add Shader"].outputs[0], new_nodes["Mix Shader.001"].inputs[1])
    tree_links.new(new_nodes["Math.001"].outputs[0], new_nodes["Math.003"].inputs[0])
    tree_links.new(new_nodes["Texture Coordinate.002"].outputs[3], new_nodes["Vector Math.004"].inputs[0])
    tree_links.new(new_nodes["Vector Math.006"].outputs[0], new_nodes["Vector Math.005"].inputs[0])
    tree_links.new(new_nodes["Vector Math.005"].outputs[1], new_nodes["Math.004"].inputs[0])
    ...
    tree_links.new(new_nodes["Geometry"].outputs[6], new_nodes["Math.005"].inputs[1])
    tree_links.new(new_nodes["Mix Shader.001"].outputs[0], new_nodes["Material Output"].inputs[0])
    tree_links.new(new_nodes["Math.005"].outputs[0], new_nodes["Mix Shader.001"].inputs[0])
...
def create_mat_pumpkin_skin(material):
    ...
    node = tree_nodes.new(type="ShaderNodeCombineXYZ")
    node.location = (3060, 760)
    node.inputs[2].default_value = 0.000000
    new_nodes["Combine XYZ.001"] = node

    node = tree_nodes.new(type="ShaderNodeVectorMath")
    node.location = (3240, 760)
    node.operation = "SUBTRACT"
    ...
    node = tree_nodes.new(type="ShaderNodeSeparateXYZ")
    node.location = (2340, 600)
    new_nodes["Separate XYZ.001"] = node

    node = tree_nodes.new(type="ShaderNodeVectorMath")
    node.location = (2340, 460)
    node.operation = "MULTIPLY"
    ...
    node = tree_nodes.new(type="ShaderNodeTexCoord")
    node.location = (1980, 600)
    node.from_instancer = False
    new_nodes["Texture Coordinate.003"] = node

    node = tree_nodes.new(type="ShaderNodeMapRange")
    node.location = (2880, 760)
    node.clamp = True
    node.data_type = "FLOAT"
    node.interpolation_type = "LINEAR"
    ...
    node = tree_nodes.new(type="ShaderNodeMixShader")
    node.location = (5180, 680)
    node.inputs[0].default_value = 0.100000
    new_nodes["Mix Shader.002"] = node

    node = tree_nodes.new(type="ShaderNodeAttribute")
    node.location = (4820, -80)
    node.attribute_name = "inside_pumpkin"
    node.attribute_type = "GEOMETRY"
    new_nodes["Attribute.005"] = node

    node = tree_nodes.new(type="ShaderNodeAttribute")
    node.location = (4820, -260)
    node.attribute_name = "top_bot_nub"
    node.attribute_type = "GEOMETRY"
    new_nodes["Attribute.006"] = node

    node = tree_nodes.new(type="ShaderNodeMixShader")
    node.location = (5360, 560)
    new_nodes["Mix Shader"] = node

    node = tree_nodes.new(type="ShaderNodeOutputMaterial")
    node.location = (5560, 580)
    node.target = "ALL"
    new_nodes["Material Output"] = node
    ...
    tree_links.new(new_nodes["Attribute.002"].outputs[2], new_nodes["Group"].inputs[0])
    tree_links.new(new_nodes["Attribute.001"].outputs[2], new_nodes["Group"].inputs[1])
    tree_links.new(new_nodes["Attribute.003"].outputs[1], new_nodes["Group"].inputs[2])
    tree_links.new(new_nodes["Mix Shader.002"].outputs[0], new_nodes["Mix Shader.001"].inputs[2])
    tree_links.new(new_nodes["Volume Absorption"].outputs[0], new_nodes["Mix Shader.002"].inputs[1])
    tree_links.new(new_nodes["Volume Scatter"].outputs[0], new_nodes["Mix Shader.002"].inputs[2])
    tree_links.new(new_nodes["Vector Math.006"].outputs[0], new_nodes["Separate XYZ.001"].inputs[0])
    ...
    tree_links.new(new_nodes["Vector Math.005"].outputs[1], new_nodes["Math.017"].inputs[1])
    tree_links.new(new_nodes["Math.017"].outputs[0], new_nodes["Map Range"].inputs[0])
    tree_links.new(new_nodes["Math.016"].outputs[0], new_nodes["Map Range.003"].inputs[0])
    tree_links.new(new_nodes["Map Range.003"].outputs[0], new_nodes["Combine XYZ.001"].inputs[0])
    tree_links.new(new_nodes["Map Range"].outputs[0], new_nodes["Combine XYZ.001"].inputs[1])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Image Texture"].inputs[0])
    tree_links.new(new_nodes["Math.018"].outputs[0], new_nodes["Mix Shader.001"].inputs[0])
    tree_links.new(new_nodes["Texture Coordinate.003"].outputs[3], new_nodes["Vector Math.006"].inputs[0])
    tree_links.new(new_nodes["Vector Math.006"].outputs[0], new_nodes["Vector Math.004"].inputs[0])
    tree_links.new(new_nodes["Attribute.005"].outputs[2], new_nodes["Group"].inputs[3])
    tree_links.new(new_nodes["Mix Shader.001"].outputs[0], new_nodes["Material Output"].inputs[1])
    tree_links.new(new_nodes["Vector Math.007"].outputs[0], new_nodes["Vector Math.009"].inputs[0])
    tree_links.new(new_nodes["Vector Math.009"].outputs[1], new_nodes["Map Range.004"].inputs[0])
    tree_links.new(new_nodes["Noise Texture.002"].outputs[0], new_nodes["Math.020"].inputs[1])
    tree_links.new(new_nodes["Math.020"].outputs[0], new_nodes["ColorRamp.005"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Vector Math.010"].inputs[0])
    tree_links.new(new_nodes["Vector Math.012"].outputs[0], new_nodes["Vector Math.011"].inputs[0])
    tree_links.new(new_nodes["Vector Math.011"].outputs[1], new_nodes["Map Range.005"].inputs[0])
    tree_links.new(new_nodes["Math.021"].outputs[0], new_nodes["Math.015"].inputs[1])
    tree_links.new(new_nodes["Math.015"].outputs[0], new_nodes["Math.020"].inputs[0])
    tree_links.new(new_nodes["Map Range.004"].outputs[0], new_nodes["Math.015"].inputs[0])
    tree_links.new(new_nodes["Vector Math.010"].outputs[0], new_nodes["Vector Math.007"].inputs[0])
    tree_links.new(new_nodes["Vector Math.010"].outputs[0], new_nodes["Vector Math.012"].inputs[0])
    tree_links.new(new_nodes["Vector Math.013"].outputs[0], new_nodes["Vector Math.014"].inputs[0])
    tree_links.new(new_nodes["Vector Math.014"].outputs[1], new_nodes["Map Range.006"].inputs[0])
    tree_links.new(new_nodes["Vector Math.010"].outputs[0], new_nodes["Vector Math.013"].inputs[0])
    tree_links.new(new_nodes["Map Range.005"].outputs[0], new_nodes["Math.021"].inputs[0])
    tree_links.new(new_nodes["Math.022"].outputs[0], new_nodes["Math.021"].inputs[1])
    tree_links.new(new_nodes["Vector Math.015"].outputs[0], new_nodes["Vector Math.016"].inputs[0])
    tree_links.new(new_nodes["Vector Math.016"].outputs[1], new_nodes["Map Range.007"].inputs[0])
    tree_links.new(new_nodes["Vector Math.010"].outputs[0], new_nodes["Vector Math.015"].inputs[0])
    tree_links.new(new_nodes["Map Range.006"].outputs[0], new_nodes["Math.022"].inputs[0])
    tree_links.new(new_nodes["Math.023"].outputs[0], new_nodes["Math.022"].inputs[1])
    tree_links.new(new_nodes["Vector Math.017"].outputs[0], new_nodes["Vector Math.018"].inputs[0])
    tree_links.new(new_nodes["Vector Math.018"].outputs[1], new_nodes["Map Range.008"].inputs[0])
    tree_links.new(new_nodes["Vector Math.010"].outputs[0], new_nodes["Vector Math.017"].inputs[0])
    tree_links.new(new_nodes["Map Range.007"].outputs[0], new_nodes["Math.023"].inputs[0])
    tree_links.new(new_nodes["Map Range.008"].outputs[0], new_nodes["Math.023"].inputs[1])
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Noise Texture.002"].inputs[0])
    tree_links.new(new_nodes["Image Texture"].outputs[0], new_nodes["Mix.002"].inputs[1])
    tree_links.new(new_nodes["ColorRamp.005"].outputs[0], new_nodes["Mix.002"].inputs[2])
    tree_links.new(new_nodes["Mix.002"].outputs[0], new_nodes["Math.018"].inputs[0])
    tree_links.new(new_nodes["Transparent BSDF.001"].outputs[0], new_nodes["Mix Shader"].inputs[1])
    tree_links.new(new_nodes["Group"].outputs[0], new_nodes["Mix Shader"].inputs[2])
    tree_links.new(new_nodes["Math.018"].outputs[0], new_nodes["Mix Shader"].inputs[0])
    tree_links.new(new_nodes["Attribute.006"].outputs[2], new_nodes["Group"].inputs[4])
...
def create_geo_ng_pumpkin():
    ...
    node.mapping.update()
    node.inputs[0].default_value = 1.000000
    new_nodes["Float Curve.003"] = node
    ...
    node = tree_nodes.new(type="NodeGroupOutput")
    node.location = (8060, -1020)
    new_nodes["Group Output"] = node

    # create links
    tree_links = new_node_group.links
    tree_links.new(new_nodes["Group Input"].outputs[14], new_nodes["Math.003"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[15], new_nodes["Math.005"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[11], new_nodes["Curve Circle"].inputs[4])
    tree_links.new(new_nodes["Group Input"].outputs[12], new_nodes["Math.010"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[13], new_nodes["Math.001"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[16], new_nodes["Math.011"].inputs[1])
    ...
    tree_links.new(new_nodes["Set Handle Type"].outputs[0], new_nodes["Set Position"].inputs[0])
    ...
    tree_links.new(new_nodes["Position.001"].outputs[0], new_nodes["Separate XYZ.001"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.001"].outputs[1], new_nodes["Math.004"].inputs[0])
    tree_links.new(new_nodes["Math.004"].outputs[0], new_nodes["Math.005"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.001"].outputs[1], new_nodes["Math"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.001"].outputs[1], new_nodes["Math.002"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.003"].outputs[0], new_nodes["Bezier Segment"].inputs[1])
    tree_links.new(new_nodes["Combine XYZ.004"].outputs[0], new_nodes["Bezier Segment"].inputs[2])
    tree_links.new(new_nodes["Math.008"].outputs[0], new_nodes["Combine XYZ.004"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.001"].outputs[1], new_nodes["Math.009"].inputs[0])
    ...
    tree_links.new(new_nodes["Set Position"].outputs[0], new_nodes["Curve to Mesh"].inputs[1])
    tree_links.new(new_nodes["Math.006"].outputs[0], new_nodes["Math.014"].inputs[0])
    tree_links.new(new_nodes["Math.013"].outputs[0], new_nodes["Math.014"].inputs[1])
    tree_links.new(new_nodes["Math.005"].outputs[0], new_nodes["Math.006"].inputs[1])
    tree_links.new(new_nodes["Math.011"].outputs[0], new_nodes["Math.013"].inputs[0])
    tree_links.new(new_nodes["Math.001"].outputs[0], new_nodes["Math.013"].inputs[1])
    tree_links.new(new_nodes["Normal"].outputs[0], new_nodes["Vector Math"].inputs[1])
    tree_links.new(new_nodes["Math.014"].outputs[0], new_nodes["Math.015"].inputs[0])
    tree_links.new(new_nodes["Math.003"].outputs[0], new_nodes["Math.015"].inputs[1])
    tree_links.new(new_nodes["Math.015"].outputs[0], new_nodes["Vector Math"].inputs[0])
    tree_links.new(new_nodes["Vector Math"].outputs[0], new_nodes["Set Position"].inputs[3])
    tree_links.new(new_nodes["Math.010"].outputs[0], new_nodes["Math.006"].inputs[0])
    tree_links.new(new_nodes["Position.002"].outputs[0], new_nodes["Separate XYZ.002"].inputs[0])
    tree_links.new(new_nodes["Math.017"].outputs[0], new_nodes["Math.016"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.002"].outputs[1], new_nodes["Math.017"].inputs[0])
    tree_links.new(new_nodes["Math.016"].outputs[0], new_nodes["Set Handle Type"].inputs[1])
    ...
    tree_links.new(new_nodes["Math.018"].outputs[0], new_nodes["Combine XYZ"].inputs[2])
    tree_links.new(new_nodes["Group Input"].outputs[12], new_nodes["Math.018"].inputs[0])
    ...
    tree_links.new(new_nodes["Combine XYZ.001"].outputs[0], new_nodes["Set Position.002"].inputs[3])
    tree_links.new(new_nodes["Math.019"].outputs[0], new_nodes["Combine XYZ.001"].inputs[2])
    tree_links.new(new_nodes["Position.003"].outputs[0], new_nodes["Vector Math.002"].inputs[0])
    tree_links.new(new_nodes["Vector Math.002"].outputs[0], new_nodes["Set Position.002"].inputs[2])
    tree_links.new(new_nodes["Set Position.002"].outputs[0], new_nodes["Join Geometry"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[11], new_nodes["Vector Math.002"].inputs[3])
    tree_links.new(new_nodes["Group Input"].outputs[16], new_nodes["Math.019"].inputs[1])
    tree_links.new(new_nodes["Separate XYZ"].outputs[0], new_nodes["Math.020"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ"].outputs[1], new_nodes["Math.020"].inputs[1])
    tree_links.new(new_nodes["Math.022"].outputs[0], new_nodes["Vector Math.003"].inputs[3])
    tree_links.new(new_nodes["Math.023"].outputs[0], new_nodes["Math.022"].inputs[0])
    tree_links.new(new_nodes["Math.020"].outputs[0], new_nodes["Math.025"].inputs[0])
    tree_links.new(new_nodes["Math.025"].outputs[0], new_nodes["Math.026"].inputs[0])
    tree_links.new(new_nodes["Math.020"].outputs[0], new_nodes["Math.026"].inputs[2])
    tree_links.new(new_nodes["Math.026"].outputs[0], new_nodes["Math.023"].inputs[0])
    tree_links.new(new_nodes["Math.024"].outputs[0], new_nodes["Vector Math.004"].inputs[3])
    tree_links.new(new_nodes["Math.027"].outputs[0], new_nodes["Math.024"].inputs[0])
    tree_links.new(new_nodes["Math.030"].outputs[0], new_nodes["Math.028"].inputs[0])
    tree_links.new(new_nodes["Math.028"].outputs[0], new_nodes["Math.029"].inputs[0])
    tree_links.new(new_nodes["Math.029"].outputs[0], new_nodes["Math.027"].inputs[0])
    tree_links.new(new_nodes["Position.005"].outputs[0], new_nodes["Vector Rotate.001"].inputs[0])
    tree_links.new(new_nodes["Math.030"].outputs[0], new_nodes["Math.029"].inputs[2])
    tree_links.new(new_nodes["Vector Rotate.001"].outputs[0], new_nodes["Set Position.003"].inputs[2])
    ...
    tree_links.new(new_nodes["Vector Math.005"].outputs[0], new_nodes["Vector Rotate.001"].inputs[4])
    tree_links.new(new_nodes["Position.005"].outputs[0], new_nodes["Separate XYZ"].inputs[0])
    tree_links.new(new_nodes["Math.020"].outputs[0], new_nodes["Math.030"].inputs[0])
    ...
    tree_links.new(new_nodes["Math.024"].outputs[0], new_nodes["Math.021"].inputs[1])
    tree_links.new(new_nodes["Math.021"].outputs[0], new_nodes["Capture Attribute"].inputs[2])
    tree_links.new(new_nodes["Capture Attribute"].outputs[2], new_nodes["Group Output"].inputs[5])
    tree_links.new(new_nodes["Merge by Distance"].outputs[0], new_nodes["Capture Attribute"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute"].outputs[0], new_nodes["Set Position.003"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.003"].outputs[0], new_nodes["Math.031"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.003"].outputs[1], new_nodes["Math.031"].inputs[1])
    tree_links.new(new_nodes["Position.006"].outputs[0], new_nodes["Separate XYZ.003"].inputs[0])
    tree_links.new(new_nodes["Normal.001"].outputs[0], new_nodes["Vector Math.006"].inputs[0])
    tree_links.new(new_nodes["Subdivision Surface"].outputs[0], new_nodes["Set Position.004"].inputs[0])
    tree_links.new(new_nodes["Vector Math.006"].outputs[0], new_nodes["Set Position.004"].inputs[3])
    tree_links.new(new_nodes["Math.036"].outputs[0], new_nodes["Float Curve"].inputs[1])
    ...
    tree_links.new(new_nodes["Math.052"].outputs[0], new_nodes["Math.035"].inputs[0])
    tree_links.new(new_nodes["Math.037"].outputs[0], new_nodes["Math.035"].inputs[1])
    tree_links.new(new_nodes["Math.037"].outputs[0], new_nodes["Math.036"].inputs[1])
    tree_links.new(new_nodes["Float Curve"].outputs[0], new_nodes["Math.038"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[17], new_nodes["Math.038"].inputs[1])
    tree_links.new(new_nodes["Position.004"].outputs[0], new_nodes["Vector Math.007"].inputs[0])
    tree_links.new(new_nodes["Vector Math.007"].outputs[0], new_nodes["Vector Math.008"].inputs[0])
    tree_links.new(new_nodes["Vector Math.008"].outputs[1], new_nodes["Math.032"].inputs[0])
    tree_links.new(new_nodes["Vector Math.008"].outputs[1], new_nodes["Math.033"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[11], new_nodes["Math.033"].inputs[1])
    tree_links.new(new_nodes["Math.033"].outputs[0], new_nodes["Float Curve.002"].inputs[1])
    tree_links.new(new_nodes["Position.004"].outputs[0], new_nodes["Separate XYZ.004"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.004"].outputs[2], new_nodes["Math.034"].inputs[0])
    tree_links.new(new_nodes["Math.034"].outputs[0], new_nodes["Math.039"].inputs[0])
    tree_links.new(new_nodes["Float Curve.003"].outputs[0], new_nodes["Math.039"].inputs[1])
    tree_links.new(new_nodes["Math.034"].outputs[0], new_nodes["Boolean Math"].inputs[0])
    tree_links.new(new_nodes["Boolean Math"].outputs[0], new_nodes["Math.040"].inputs[0])
    tree_links.new(new_nodes["Float Curve.002"].outputs[0], new_nodes["Math.040"].inputs[1])
    tree_links.new(new_nodes["Math.040"].outputs[0], new_nodes["Math.041"].inputs[1])
    tree_links.new(new_nodes["Math.039"].outputs[0], new_nodes["Math.041"].inputs[0])
    tree_links.new(new_nodes["Math.041"].outputs[0], new_nodes["Math.042"].inputs[1])
    tree_links.new(new_nodes["Math.038"].outputs[0], new_nodes["Math.042"].inputs[0])
    tree_links.new(new_nodes["Math.042"].outputs[0], new_nodes["Vector Math.006"].inputs[3])
    tree_links.new(new_nodes["Math.032"].outputs[0], new_nodes["Float Curve.003"].inputs[1])
    tree_links.new(new_nodes["Join Geometry"].outputs[0], new_nodes["Capture Attribute.003"].inputs[0])
    tree_links.new(new_nodes["Math.043"].outputs[0], new_nodes["Capture Attribute.003"].inputs[2])
    tree_links.new(new_nodes["Capture Attribute.002"].outputs[2], new_nodes["Math.043"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.001"].outputs[2], new_nodes["Math.043"].inputs[1])
    tree_links.new(new_nodes["Mesh Circle"].outputs[0], new_nodes["Capture Attribute.002"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.002"].outputs[0], new_nodes["Set Position.001"].inputs[0])
    tree_links.new(new_nodes["Flip Faces"].outputs[0], new_nodes["Capture Attribute.001"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.001"].outputs[0], new_nodes["Set Position.002"].inputs[0])
    ...
    tree_links.new(new_nodes["Math.044"].outputs[0], new_nodes["Combine XYZ.002"].inputs[1])
    tree_links.new(new_nodes["Math.045"].outputs[0], new_nodes["Combine XYZ.002"].inputs[0])
    tree_links.new(new_nodes["Math.046"].outputs[0], new_nodes["Math.045"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.002"].outputs[0], new_nodes["Vector Math.009"].inputs[0])
    tree_links.new(new_nodes["Curve to Mesh.001"].outputs[0], new_nodes["Join Geometry.001"].inputs[0])
    tree_links.new(new_nodes["Set Position.005"].outputs[0], new_nodes["Join Geometry.001"].inputs[0])
    tree_links.new(new_nodes["Flip Faces.001"].outputs[0], new_nodes["Join Geometry.001"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.005"].outputs[0], new_nodes["Set Position.005"].inputs[3])
    tree_links.new(new_nodes["Vector Math.009"].outputs[0], new_nodes["Bezier Segment.001"].inputs[2])
    tree_links.new(new_nodes["Combine XYZ.002"].outputs[0], new_nodes["Bezier Segment.001"].inputs[1])
    tree_links.new(new_nodes["Merge by Distance.001"].outputs[0], new_nodes["Set Position.006"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[7], new_nodes["Math.047"].inputs[0])
    tree_links.new(new_nodes["Math.047"].outputs[0], new_nodes["Math.046"].inputs[0])
    tree_links.new(new_nodes["Math.047"].outputs[0], new_nodes["Curve Circle.001"].inputs[4])
    tree_links.new(new_nodes["Group Input"].outputs[5], new_nodes["Math.046"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[5], new_nodes["Mesh Circle.002"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[6], new_nodes["Math.044"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[6], new_nodes["Combine XYZ.005"].inputs[2])
    tree_links.new(new_nodes["Combine XYZ.006"].outputs[0], new_nodes["Vector Math.009"].inputs[1])
    tree_links.new(new_nodes["Math.048"].outputs[0], new_nodes["Combine XYZ.006"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[6], new_nodes["Math.048"].inputs[0])
    ...
    tree_links.new(new_nodes["Set Position.006"].outputs[0], new_nodes["Attribute Statistic"].inputs[0])
    tree_links.new(new_nodes["Position.008"].outputs[0], new_nodes["Separate XYZ.006"].inputs[0])
    tree_links.new(new_nodes["Attribute Statistic"].outputs[3], new_nodes["Map Range"].inputs[1])
    tree_links.new(new_nodes["Attribute Statistic"].outputs[4], new_nodes["Map Range"].inputs[2])
    tree_links.new(new_nodes["Separate XYZ.006"].outputs[2], new_nodes["Map Range"].inputs[0])
    tree_links.new(new_nodes["Map Range"].outputs[0], new_nodes["Capture Attribute.004"].inputs[2])
    ...
    tree_links.new(new_nodes["Vector Math.012"].outputs[0], new_nodes["Capture Attribute.005"].inputs[1])
    tree_links.new(new_nodes["Capture Attribute.005"].outputs[1], new_nodes["Group Output"].inputs[3])
    tree_links.new(new_nodes["Set Position.006"].outputs[0], new_nodes["Capture Attribute.005"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.005"].outputs[0], new_nodes["Capture Attribute.004"].inputs[0])
    tree_links.new(new_nodes["Set Position.007"].outputs[0], new_nodes["Set Material.001"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.004"].outputs[0], new_nodes["Set Position.007"].inputs[0])
    tree_links.new(new_nodes["Position.010"].outputs[0], new_nodes["Vector Rotate"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.004"].outputs[2], new_nodes["Math.049"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.008"].outputs[0], new_nodes["Vector Rotate"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[6], new_nodes["Math.050"].inputs[0])
    tree_links.new(new_nodes["Math.051"].outputs[0], new_nodes["Combine XYZ.009"].inputs[2])
    tree_links.new(new_nodes["Combine XYZ.009"].outputs[0], new_nodes["Set Position.006"].inputs[3])
    tree_links.new(new_nodes["Math.018"].outputs[0], new_nodes["Math.051"].inputs[0])
    tree_links.new(new_nodes["Math.018"].outputs[0], new_nodes["Combine XYZ.008"].inputs[2])
    tree_links.new(new_nodes["Math.050"].outputs[0], new_nodes["Combine XYZ.008"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[9], new_nodes["Math.049"].inputs[1])
    tree_links.new(new_nodes["Vector Rotate"].outputs[0], new_nodes["Set Position.007"].inputs[2])
    tree_links.new(new_nodes["Math.049"].outputs[0], new_nodes["Combine XYZ.007"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.007"].outputs[0], new_nodes["Vector Rotate.002"].inputs[0])
    tree_links.new(new_nodes["Vector Rotate.002"].outputs[0], new_nodes["Vector Rotate"].inputs[4])
    tree_links.new(new_nodes["Combine XYZ.010"].outputs[0], new_nodes["Vector Rotate.002"].inputs[4])
    tree_links.new(new_nodes["Group Input"].outputs[8], new_nodes["Combine XYZ.010"].inputs[2])
    tree_links.new(new_nodes["Set Position.003"].outputs[0], new_nodes["Set Material"].inputs[0])
    ...
    tree_links.new(new_nodes["Separate XYZ.008"].outputs[2], new_nodes["Attribute Statistic.001"].inputs[2])
    tree_links.new(new_nodes["Position.012"].outputs[0], new_nodes["Separate XYZ.007"].inputs[0])
    tree_links.new(new_nodes["Attribute Statistic.001"].outputs[3], new_nodes["Map Range.001"].inputs[1])
    tree_links.new(new_nodes["Attribute Statistic.001"].outputs[4], new_nodes["Map Range.001"].inputs[2])
    tree_links.new(new_nodes["Separate XYZ.007"].outputs[2], new_nodes["Map Range.001"].inputs[0])
    tree_links.new(new_nodes["Map Range.001"].outputs[0], new_nodes["Capture Attribute.008"].inputs[2])
    tree_links.new(new_nodes["Position.011"].outputs[0], new_nodes["Vector Math.013"].inputs[0])
    tree_links.new(new_nodes["Vector Math.013"].outputs[0], new_nodes["Vector Math.014"].inputs[0])
    tree_links.new(new_nodes["Vector Math.014"].outputs[0], new_nodes["Capture Attribute.007"].inputs[1])
    tree_links.new(new_nodes["Capture Attribute.008"].outputs[2], new_nodes["Group Output"].inputs[7])
    tree_links.new(new_nodes["Capture Attribute.007"].outputs[1], new_nodes["Group Output"].inputs[8])
    ...
    tree_links.new(new_nodes["Math.031"].outputs[0], new_nodes["Math.052"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[0], new_nodes["Map Range.003"].inputs[0])
    tree_links.new(new_nodes["Map Range.003"].outputs[0], new_nodes["Subdivision Surface"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[0], new_nodes["Map Range.004"].inputs[0])
    tree_links.new(new_nodes["Map Range.004"].outputs[0], new_nodes["Subdivision Surface.001"].inputs[1])
    tree_links.new(new_nodes["Capture Attribute.004"].outputs[2], new_nodes["Math.053"].inputs[0])
    ...
    tree_links.new(new_nodes["Flip Faces.002"].outputs[0], new_nodes["Set Material.003"].inputs[0])
    tree_links.new(new_nodes["Normal.002"].outputs[0], new_nodes["Vector Math.010"].inputs[0])
    tree_links.new(new_nodes["Vector Math.010"].outputs[0], new_nodes["Set Position.008"].inputs[3])
    tree_links.new(new_nodes["Group Input"].outputs[18], new_nodes["Vector Math.010"].inputs[3])
    tree_links.new(new_nodes["Capture Attribute.009"].outputs[0], new_nodes["Capture Attribute.007"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.008"].outputs[0], new_nodes["Capture Attribute.009"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.009"].outputs[2], new_nodes["Group Output"].inputs[9])
    tree_links.new(new_nodes["Attribute Statistic.002"].outputs[4], new_nodes["Map Range.005"].inputs[2])
    tree_links.new(new_nodes["Vector Math.016"].outputs[0], new_nodes["Vector Math.015"].inputs[0])
    tree_links.new(new_nodes["Vector Math.015"].outputs[1], new_nodes["Map Range.005"].inputs[0])
    tree_links.new(new_nodes["Position.015"].outputs[0], new_nodes["Vector Math.016"].inputs[0])
    tree_links.new(new_nodes["Map Range.005"].outputs[0], new_nodes["Capture Attribute.009"].inputs[2])
    tree_links.new(new_nodes["Vector Math.017"].outputs[0], new_nodes["Vector Math.018"].inputs[0])
    tree_links.new(new_nodes["Position.016"].outputs[0], new_nodes["Vector Math.017"].inputs[0])
    tree_links.new(new_nodes["Vector Math.018"].outputs[1], new_nodes["Attribute Statistic.002"].inputs[2])
    tree_links.new(new_nodes["Mesh Circle.002"].outputs[0], new_nodes["Set Position.005"].inputs[0])
    tree_links.new(new_nodes["Position.014"].outputs[0], new_nodes["Separate XYZ.009"].inputs[0])
    tree_links.new(new_nodes["Separate XYZ.009"].outputs[2], new_nodes["Attribute Statistic.003"].inputs[2])
    tree_links.new(new_nodes["Math.057"].outputs[0], new_nodes["Combine XYZ.011"].inputs[2])
    tree_links.new(new_nodes["Combine XYZ.011"].outputs[0], new_nodes["Set Position.009"].inputs[3])
    tree_links.new(new_nodes["Group Input"].outputs[19], new_nodes["Math.055"].inputs[1])
    tree_links.new(new_nodes["Math.055"].outputs[0], new_nodes["Math.057"].inputs[0])
    tree_links.new(new_nodes["Attribute Statistic.003"].outputs[3], new_nodes["Math.057"].inputs[1])
    tree_links.new(new_nodes["Combine XYZ.011"].outputs[0], new_nodes["Set Position.010"].inputs[3])
    tree_links.new(new_nodes["Subdivision Surface.001"].outputs[0], new_nodes["Set Position.010"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.007"].outputs[0], new_nodes["Attribute Statistic.003"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.007"].outputs[0], new_nodes["Set Position.009"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[1], new_nodes["Switch"].inputs[1])
    tree_links.new(new_nodes["Set Position.004"].outputs[0], new_nodes["Attribute Statistic.002"].inputs[0])
    tree_links.new(new_nodes["Set Position.004"].outputs[0], new_nodes["Capture Attribute.008"].inputs[0])
    tree_links.new(new_nodes["Set Position.004"].outputs[0], new_nodes["Attribute Statistic.001"].inputs[0])
    tree_links.new(new_nodes["Combine XYZ.011"].outputs[0], new_nodes["Set Position.011"].inputs[3])
    tree_links.new(new_nodes["Set Position.008"].outputs[0], new_nodes["Set Position.011"].inputs[0])
    tree_links.new(new_nodes["Vector Math.020"].outputs[0], new_nodes["Set Position.012"].inputs[2])
    tree_links.new(new_nodes["Group Input"].outputs[2], new_nodes["Vector Math.020"].inputs[1])
    tree_links.new(new_nodes["Vector Math.020"].outputs[0], new_nodes["Set Position.013"].inputs[2])
    tree_links.new(new_nodes["Vector Math.020"].outputs[0], new_nodes["Set Position.014"].inputs[2])
    tree_links.new(new_nodes["Set Position.011"].outputs[0], new_nodes["Set Position.014"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.006"].outputs[0], new_nodes["Group Output"].inputs[11])
    tree_links.new(new_nodes["Set Position.009"].outputs[0], new_nodes["Set Position.013"].inputs[0])
    tree_links.new(new_nodes["Set Position.013"].outputs[0], new_nodes["Group Output"].inputs[4])
    tree_links.new(new_nodes["Set Position.010"].outputs[0], new_nodes["Set Position.012"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.010"].outputs[0], new_nodes["Group Output"].inputs[0])
    tree_links.new(new_nodes["Vector Math.021"].outputs[0], new_nodes["Vector Math.020"].inputs[0])
    tree_links.new(new_nodes["Position.017"].outputs[0], new_nodes["Vector Math.021"].inputs[0])
    tree_links.new(new_nodes["Set Position.014"].outputs[0], new_nodes["Capture Attribute.006"].inputs[0])
    tree_links.new(new_nodes["Capture Attribute.006"].outputs[2], new_nodes["Group Output"].inputs[12])
    tree_links.new(new_nodes["Set Position.012"].outputs[0], new_nodes["Capture Attribute.010"].inputs[0])
    ...
    tree_links.new(new_nodes["Set Position.012"].outputs[0], new_nodes["Attribute Statistic.004"].inputs[0])
    tree_links.new(new_nodes["Attribute Statistic.003"].outputs[5], new_nodes["Math.055"].inputs[0])
    ...
    tree_links.new(new_nodes["Set Material.003"].outputs[0], new_nodes["Set Shade Smooth"].inputs[0])
    ...
    tree_links.new(new_nodes["Group Input"].outputs[3], new_nodes["Set Material.003"].inputs[2])
    tree_links.new(new_nodes["Group Input"].outputs[10], new_nodes["Vector Math.001"].inputs[3])
    tree_links.new(new_nodes["Group Input"].outputs[10], new_nodes["Math.032"].inputs[1])
    tree_links.new(new_nodes["Group Input"].outputs[11], new_nodes["Math.008"].inputs[0])
    tree_links.new(new_nodes["Group Input"].outputs[11], new_nodes["Combine XYZ.003"].inputs[0])
...
def create_apply_pumpkin_geo_nodes(new_node_group, tree_nodes, tree_links, pumpkin_obj):
    ...
    node.noise_dimensions = "4D"
    node.inputs[1].default_value = -3.149999
    node.inputs[2].default_value = 6.000000
    node.inputs[3].default_value = 2.000000
    node.inputs[4].default_value = 0.500000
    node.inputs[5].default_value = 0.000000
    ...
    node.inputs[5].default_value = 0.050000
    node.inputs[7].default_value = 0.080000
    node.inputs[10].default_value = 0.030000
    node.inputs[11].default_value = 0.020000
    node.inputs[14].default_value = 0.000000
    node.inputs[17].default_value = 0.050000
    node.inputs[18].default_value = 0.080000
    node.inputs[19].default_value = 0.000000
    new_nodes["Group"] = node
    ...
    tree_links.new(new_nodes["Join Geometry"].outputs[0], new_nodes["Set Position.001"].inputs[0])
    tree_links.new(new_nodes["Group.001"].outputs[0], new_nodes["Set Position.001"].inputs[3])
    ...
    tree_links.new(new_nodes["ColorRamp"].outputs[0], new_nodes["Math"].inputs[0])
    tree_links.new(new_nodes["Math"].outputs[0], new_nodes["Group.001"].inputs[2])
    tree_links.new(new_nodes["Noise Texture.001"].outputs[0], new_nodes["Math.001"].inputs[0])
    tree_links.new(new_nodes["Math.001"].outputs[0], new_nodes["Group.001"].inputs[3])
    tree_links.new(new_nodes["Map Range"].outputs[0], new_nodes["Math.002"].inputs[1])
    tree_links.new(new_nodes["ColorRamp.001"].outputs[0], new_nodes["Math.002"].inputs[0])
    tree_links.new(new_nodes["Math.002"].outputs[0], new_nodes["Group.001"].inputs[5])
    tree_links.new(new_nodes["Math.002"].outputs[0], new_nodes["Group.001"].inputs[4])
    ...
    tree_links.new(new_nodes["Separate RGB"].outputs[0], new_nodes["Map Range.001"].inputs[0])
    tree_links.new(new_nodes["Map Range.001"].outputs[0], new_nodes["Group"].inputs[12])
    tree_links.new(new_nodes["Separate RGB"].outputs[1], new_nodes["Map Range.002"].inputs[0])
    tree_links.new(new_nodes["Separate RGB"].outputs[2], new_nodes["Map Range.003"].inputs[0])
    tree_links.new(new_nodes["Map Range.003"].outputs[0], new_nodes["Group"].inputs[16])
    tree_links.new(new_nodes["Map Range.001"].outputs[0], new_nodes["Group"].inputs[13])
    tree_links.new(new_nodes["Map Range.002"].outputs[0], new_nodes["Group"].inputs[15])
    ...
    tree_links.new(new_nodes["Map Range.004"].outputs[0], new_nodes["Group"].inputs[6])
    tree_links.new(new_nodes["Set Position.001"].outputs[0], new_nodes["Set Position.002"].inputs[0])
    tree_links.new(new_nodes["Position.001"].outputs[0], new_nodes["Vector Math.001"].inputs[0])
    tree_links.new(new_nodes["Vector Math.001"].outputs[0], new_nodes["Set Position.002"].inputs[2])
    tree_links.new(new_nodes["Noise Texture.001"].outputs[0], new_nodes["Map Range.005"].inputs[0])
    tree_links.new(new_nodes["Map Range.005"].outputs[0], new_nodes["Vector Math.001"].inputs[3])
    tree_links.new(new_nodes["Noise Texture.001"].outputs[0], new_nodes["Map Range.006"].inputs[0])
    tree_links.new(new_nodes["Map Range.006"].outputs[0], new_nodes["Group"].inputs[8])
    tree_links.new(new_nodes["Noise Texture.001"].outputs[0], new_nodes["Map Range.007"].inputs[0])
    tree_links.new(new_nodes["Map Range.007"].outputs[0], new_nodes["Group"].inputs[9])
...
def create_pumpkin_materials(override_create, pumpkin_obj):
    ...
    ensure_materials(override_create, [PUMPKIN_SKIN_MAT_NAME, PUMPKIN_STEM_MAT_NAME], create_prereq_material)
